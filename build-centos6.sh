...
echo "echo 'python bazel_configure.py' | scl enable python27 bash" >> docker_command.txt;
echo "echo 'bazel build -c opt --config=centos scripts/packages:binpkgs' | scl enable python27 bash" >> docker_command.txt;
echo "echo 'bazel build -c opt --config=centos scripts/packages:tarpkgs' | scl enable python27 bash" >> docker_command.txt;
