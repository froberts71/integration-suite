...
echo "echo 'bazel build -c opt --config=centos scripts/packages:binpkgs' | scl enable devtoolset-3 python27 bash" >> docker_command.txt;
echo "echo 'bazel build -c opt --config=centos scripts/packages:tarpkgs' | scl enable devtoolset-3 python27 bash" >> docker_command.txt;
