...
echo "scl enable python27 bash && python bazel_configure.py" >> docker_command.txt;
echo "scl enable python27 bash && bazel build -c opt --config=centos scripts/packages:binpkgs" >> docker_command.txt;
echo "scl enable python27 bash && bazel build -c opt --config=centos scripts/packages:tarpkgs" >> docker_command.txt;
