...
echo "bazel build -c opt --config=darwin scripts/packages:binpkgs" >> docker_command.txt;
echo "bazel build -c opt --config=darwin scripts/packages:tarpkgs" >> docker_command.txt;
