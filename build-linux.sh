...
echo "bazel build -c opt --config=linux scripts/packages:binpkgs" >> docker_command.txt;
echo "bazel build -c opt --config=linux scripts/packages:tarpkgs" >> docker_command.txt;
